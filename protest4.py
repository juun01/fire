...
# 모델 로드
model = YOLO("/home/pi/Downloads/best (3).pt")
model.fuse()

save_directory = " fire_capture"
...
def inference_thread():
    ...
    while True:
        ...
        if frame is None:
            continue
        result = model(frame, imgsz=416, conf=0.3, verbose=False)
        with result_lock:
...
def get_cpu_temp():
    ...
    return temp_output.strip()

# 온도 모니터링 스레드 함수
def temp_monitor():
    process = psutil.Process()
    process.cpu_percent(interval=None)  # 초기화
    ...
    try:
        while True:
            ...
            percent_of_total = (proc_cpu / (cpu_count * 100)) * 100

            print(f"[자원 모니터링] Memory: {mem:.2f} MB, CPU:  {percent_of_total:.2f}%")
